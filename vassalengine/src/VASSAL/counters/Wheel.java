...
public class Wheel extends Decorator implements TranslatablePiece {
  ...
  protected int wheelX;
  protected int wheelY;
  protected double angle;
  protected String command;
  protected NamedKeyStroke key;
  protected boolean fixedAngle;
  protected KeyCommand[] commands;
  protected KeyCommand wheelCommand;
  ...
  protected KeyCommand[] myGetKeyCommands() {
    if (commands == null) {
      wheelCommand = new KeyCommand(command, key, Decorator.getOutermost(this), this);
      if (command.length() > 0 && key != null && !key.isNull()) {
        commands = new KeyCommand[]{wheelCommand};
      }
      else {
        commands = new KeyCommand[0];
      }
      rotator = (FreeRotator) Decorator.getDecorator(this, FreeRotator.class);
      wheelCommand.setEnabled(rotator != null);
    }
    return commands;
  }
  ...
  public Command myKeyEvent(KeyStroke stroke) {
    myGetKeyCommands();
    Command c = null;
    if (wheelCommand.matches(stroke)) {
      if (fixedAngle) {
        ...
        if (getMap() != null) {
          ...
          wheelPoint(pos, -Math.PI * oldAngle / 180.0, -Math.PI * newAngle / 180.0);
        ...
        Point2D wheel2D = new Point2D.Double(wheelX, wheelY);
        AffineTransform t = AffineTransform.getRotateInstance(oldAngle);
        t.transform(wheel2D, wheel2D);
        rotator.beginInteractiveRotate();
        rotator.setPivot(getPosition().x + (int) Math.round(wheel2D.getX()),
                         getPosition().y + (int) Math.round(wheel2D.getY()));
      }
    }
  ...
  /**
   * Wheel the given point around the wheel point from oldAngle to newAngle
   * @param oldAngle
   * @param newAngle
   */
  private void wheelPoint(Point p, double oldAngle, double newAngle) {
    Point2D wheel2D = new Point2D.Double(wheelX, wheelY);
    AffineTransform t = AffineTransform.getRotateInstance(oldAngle);
    t.transform(wheel2D, wheel2D);
    t = AffineTransform.getRotateInstance(newAngle - oldAngle, wheel2D.getX(), wheel2D.getY());
  ...
  public PieceI18nData getI18nData() {
    return getI18nData(command, "Wheel command");
  }
  ...
  public static class Ed implements PieceEditor {
    ...
    public Ed(Wheel p) {
      ...
      xOff = new IntConfigurer(null, "Wheel point:  ", p.wheelX);
      b.add(xOff.getControls());
      yOff = new IntConfigurer(null, ", ", p.wheelY);
      b.add(yOff.getControls());
      controls.add(b);

      fixedAngle = new BooleanConfigurer(null, "Wheel through fixed angle?",
